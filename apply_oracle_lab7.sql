...
-- Call library files.
@/home/student/lab6/apply_oracle_lab6.sql
...
-- ----------------------------------------------------------------------
--  Step #2: Verify the common_lookup contents.
-- ----------------------------------------------------------------------
COLUMN common_lookup_table  FORMAT A20 HEADING "COMMON_LOOKUP_TABLE"
COLUMN common_lookup_column FORMAT A20 HEADING "COMMON_LOOKUP_COLUMN"
COLUMN common_lookup_type   FORMAT A20 HEADING "COMMON_LOOKUP_TYPE"
...
,        CASE
           WHEN  (TRUNC(SYSDATE) - i.release_date) > 30 AND active_flag = 'Y' THEN i.release_date + 31
           ELSE  i.release_date
         END AS start_date
,        CASE
           WHEN  (TRUNC(SYSDATE) - i.release_date) > 30 AND active_flag = 'N' THEN i.release_date + 30
         END AS end_date
,        CASE
           WHEN  (TRUNC(SYSDATE) - i.release_date) > 30 AND active_flag = 'Y' THEN 
           CASE
...
AND NOT ( (TRUNC(SYSDATE) - i.release_date) <= 30 AND active_flag = 'N')
ORDER BY 1, 2, 3;
